...
static esp_err_t ota_manifest_check_handler(httpd_req_t *req) {
	...
	cJSON_AddItemToObject(resp, "manifest", manifest); /* transfers ownership */
...
static esp_err_t ota_pull_handler(httpd_req_t *req) {
	...
	/* ── Version check — skip flash if already on this version ── */
	const esp_app_desc_t *running = esp_app_get_description();
	...
	/* esp_ota_end verifies the SHA-256 appended to every ESP-IDF image */
	err = esp_ota_end(ota_handle);
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "%s: esp_ota_end failed: %s", __func__, esp_err_to_name(err));
		httpd_resp_set_status(req, "500 Internal Server Error");
...
esp_err_t start_server(const char *base_path, int port) {
	...
	httpd_uri_t _options_ota_manifest_handler = {
		.uri = "/api/ota/manifest",
		.method = HTTP_OPTIONS,
		.handler = options_handler,
	};
	httpd_register_uri_handler(server, &_options_ota_manifest_handler);

	httpd_uri_t _ota_pull_handler = {
		.uri = "/api/ota/pull",
		.method = HTTP_POST,
		.handler = ota_pull_handler,
	};
	httpd_register_uri_handler(server, &_ota_pull_handler);

	httpd_uri_t _options_ota_pull_handler = {
		.uri = "/api/ota/pull",
		.method = HTTP_OPTIONS,
		.handler = options_handler,
	};
	httpd_register_uri_handler(server, &_options_ota_pull_handler);
#endif /* CONFIG_SNAPCLIENT_WEB_OTA_PULL */
#endif /* CONFIG_SNAPCLIENT_WEB_OTA */
