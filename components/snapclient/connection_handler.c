...
void setup_network(esp_netif_t **netif) {
	...
	CONFIG_SNAPCLIENT_USE_SPI_ETHERNET
		 ...
		 * The settings_manager holds the mdns flag and optional server
		 * host/port.
		 */
		ip_addr_t remote_ip;
		bool use_mdns = true;
		...
		if (use_mdns) {
			...
			if (settings_get_server_host(static_host, sizeof(static_host)) !=
				...
				static_host[0] == '\0') {
				ESP_LOGW(TAG,
						 "Static server not configured in settings, skipping");
				vTaskDelay(pdMS_TO_TICKS(1000));
				continue;
			}
			...
			if (settings_get_server_port(&static_port) != ESP_OK) {
				ESP_LOGW(TAG,
						 "Failed to read static server port from settings");
				vTaskDelay(pdMS_TO_TICKS(1000));
				continue;
			}
			...
			if (ipaddr_aton(static_host, &remote_ip) == 0) {
				ESP_LOGE(TAG,
						 "can't convert static server address to numeric: %s",
						 static_host);
				continue;
			}
...
#define USE_INTERFACE_BIND
		...
		if (rc1 != ERR_OK) {
			ESP_LOGE(TAG, "can't bind interface %s",
...
static int connection_ensure_byte(connection_t *connection) {
	...
	while (1) {
		...
		case CONNECTION_DATA_RECEIVED: {
			if (fill_buffer(&connection->first_netbuf_processed,
							&connection->rc1, connection->firstNetBuf,
