...
void time_sync_msg_cb(void *args) {
	...
	//  uint8_t *p_pkt = (uint8_t *)malloc(BASE_MESSAGE_SIZE +
	//  TIME_MESSAGE_SIZE); if (p_pkt == NULL) {
	//    ESP_LOGW(
	//        TAG,
	...
	rc1 = netconn_write(lwipNetconn, p_pkt,
						BASE_MESSAGE_SIZE + TIME_MESSAGE_SIZE, NETCONN_NOCOPY);
...
write_callback(const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame,
	...
	do {
		...
		if (!pcmData) {
			ESP_LOGW(TAG,
					 "%s, failed to allocate PCM chunk payload (%lu + %u "
					 "bytes, free "
	...
	for (i = 0; i < frame->header.blocksize; i++) {
		...
		pcmChunk.outData[pcmChunk.bytes + 4 * i + 1] =
			(uint8_t)(buffer[0][i] >> 8);
		pcmChunk.outData[pcmChunk.bytes + 4 * i + 2] = (uint8_t)(buffer[1][i]);
		pcmChunk.outData[pcmChunk.bytes + 4 * i + 3] =
			(uint8_t)(buffer[1][i] >> 8);
	}
	...
	if (codec == OPUS) {
		...
		FLAC__StreamDecoderInitStatus init_status =
			FLAC__stream_decoder_init_stream(
				flacDecoder, read_callback, NULL, NULL, NULL, NULL,
				write_callback, metadata_callback, error_callback, scSet);
...
void handle_chunk_message(codec_type_t codec, snapcastSetting_t *scSet,
	...
	case OPUS: {
		...
		do {
			...
			frame_size = opus_decode(opusDecoder, decoderChunk.inData,
	...
	case FLAC: {
		...
		if ((cachedBlocks > 0) && (scSet->sr != 0)) {
			...
			uint64_t timestamp = 1000000ULL * wire_chnk->timestamp.sec +
								 wire_chnk->timestamp.usec;
		...
		if (ret == 0) {
			...
			if (fragment->payload != NULL) {
				uint32_t frames =
					pcmChunk.bytes / (scSet->ch * (scSet->bits / 8));
				...
				for (int i = 0; i < frames; i++) {
					...
					if (fragment != NULL) {
						volatile uint32_t *test = (volatile uint32_t *)(&(
...
int process_data(snapcast_protocol_parser_t *parser,
	...
	case SNAPCAST_MESSAGE_WIRE_CHUNK: {
		...
		// skip this wires chunk message if codec header message was not
		// received yet!
...
static void http_get_task(void *pvParameters) {
	...
	while (1) {
		...
		//   ESP_LOGE(TAG,
		//            "reset_diff_buffer: couldn't reset median filter long.
		//            STOP");
		//   return;
		// }
	...
	CONFIG_SNAPCLIENT_USE_SPI_ETHERNET
		...
		before_receive_callback_data.received_codec_header =
		...
		// as we need fast time syncs in the beginning we set receive timeout
		// very low
		time_sync_data.timeout = FAST_SYNC_LATENCY_BUF;
		netconn_set_recvtimeout(lwipNetconn,
